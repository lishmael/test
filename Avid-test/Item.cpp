...
void Item::process() {
    // If this item was already processed, just return
    if (isReady()) return;
    ...
    // If we succeed to open file, continue processing
    // Else, exit with state BAD
    if (!inFile.is_open()) {
    ...
    // Get file handle from winapi, by opening it
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
        // Collect file data from winapi if file handle opens successfully
    ...
    // Transform file size in human-readable form
